...
def process_user_data(ig_name, rolling_window):
    ...
    hourly_total_likes = filtered_df.set_index('comment_datetime').resample('d')['likes'].sum()
    
    filtered_df['likes_relativity'] = filtered_df.apply(lambda row: row['likes'] / hourly_total_likes[row['comment_datetime'].floor('d')] if row['comment_datetime'].floor('h') in hourly_total_likes.index else 0, axis=1)
    
    filtered_df['weighted_sentiment'] = filtered_df['sentiment'] * filtered_df['likes_relativity']
    
    hourly_sentiment = filtered_df.set_index('comment_datetime').resample('d').agg({
        'sentiment': 'mean',
